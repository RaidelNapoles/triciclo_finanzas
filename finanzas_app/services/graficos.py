...
class GeneradorGraficos:
    ...
    @staticmethod
    def crear_grafico_semanal(datos_semanales, max_semanas=12):
        """
        ...
        for semana in datos_a_mostrar:
            ...
            if "año" in semana:
                etiqueta = f"{semana['año']}-{semana['numero_semana']:02d}"
            semanas.append(etiqueta)
            ingresos.append(float(semana["total_ingresos"]))
    ...
    @staticmethod
    def crear_grafico_diario(promedios_diarios):
        """
        ...
        for nombre_dia in dias_orden:
            for datos_dia in promedios_diarios:
                if datos_dia["nombre_dia"] == nombre_dia:
                    dias.append(nombre_dia[:3])  # Abreviar a 3 letras
                    promedios.append(float(datos_dia["promedio_ingresos"]))
                    break
            else:
                dias.append(nombre_dia[:3])
                promedios.append(0)
        ...
        # Configurar eje X
        ax.set_xlabel(
            "DÍA DE LA SEMANA", fontdict=CONFIG_GRAFICOS["fuentes"]["ejes"], labelpad=20
        )

        # Configurar eje Y
        ax.set_ylabel(
            "PROMEDIO DE INGRESOS ($)",
            fontdict=CONFIG_GRAFICOS["fuentes"]["ejes"],
            labelpad=20,
        )
    ...
    def crear_grafico_mensual(datos_mensuales, max_meses=12):
        ...
        for datos_mes in datos_a_mostrar:
            ...
            ingresos.append(float(datos_mes["total_ingresos"]))
    ...
    @staticmethod
    def _agregar_etiquetas_barras(ax, barras):
        """
        ...
        for barra in barras:
            ...
            if altura > 0:  # Solo mostrar etiquetas para valores positivos
                ...
                ax.annotate(
                    ...
                    ha="center",
                    va="bottom",
                    fontsize=CONFIG_GRAFICOS["fuentes"]["numeros"]["size"],
